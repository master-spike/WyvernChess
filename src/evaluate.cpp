...
constexpr int place_value_pawn[64] = {
    ...
    5,  -5, -5, 0,  0,  -10, -5, 5,  0,  0,  5,  20,  20,  0,  0,  0,
...
constexpr int place_value_bishop[64] = {
    ...
    0,   0,   -10, -10, 10,  10,  10,  10,  10,  10,  -10, -10, 0,
...
int Evaluator::evalPositional(Position const &pos) {
  ...
  U64 our_pawn_cs =
      (player) ? (pawns_black >> 7 & ~FILE_A) | (pawns_black >> 9 & ~FILE_H)
               : (pawns_white << 7 & ~FILE_H) | (pawns_white << 9 & ~FILE_A);
  U64 opp_pawn_cs =
      (opponent) ? (pawns_black >> 7 & ~FILE_A) | (pawns_black >> 9 & ~FILE_H)
                 : (pawns_white << 7 & ~FILE_H) | (pawns_white << 9 & ~FILE_A);

  int endgame_interp =
      (total_material <= endgame_material_limit) ? 0
      : (total_material >= midgame_material_limit)
          ? eg_mg_diff
          : (eg_mg_diff * (total_material - endgame_material_limit)) /
                eg_mg_diff;
  ...
  for (U64 pawns = pcs[PAWN - 1] & pcols[player]; pawns; pawns &= pawns - 1) {
    ...
    if (!(mt->passed_pawns[p + 64 * player] & pcs[PAWN - 1] & pcols[opponent]))
      total += passed_pawn_value;
  }
  for (U64 pawns = pcs[PAWN - 1] & pcols[opponent]; pawns; pawns &= pawns - 1) {
    int p = std::countr_zero(pawns);
    total -= psqvTableLookup(opponent, p, place_value_pawn);
    if (!(mt->passed_pawns[p + 64 * opponent] & pcs[PAWN - 1] & pcols[player]))
      total -= passed_pawn_value;
  }

  for (U64 knights = pcs[KNIGHT - 1] & pcols[player]; knights;
       knights &= knights - 1) {
    int p = std::countr_zero(knights);
    total += psqvTableLookup(player, p, place_value_knight);
    U64 targets = mt->knight_table[p] & ~opp_pawn_cs;
    total +=
        (std::popcount(targets) * knight_mobility_factor * endgame_interp) /
        eg_mg_diff;
  }
  for (U64 knights = pcs[KNIGHT - 1] & pcols[opponent]; knights;
       knights &= knights - 1) {
    int p = std::countr_zero(knights);
  ...
  for (U64 bishops = pcs[BISHOP - 1] & pcols[player]; bishops;
       bishops &= bishops - 1) {
    ...
    total +=
        (std::popcount(targets) * bishop_mobility_factor * endgame_interp) /
        eg_mg_diff;
  }
  for (U64 bishops = pcs[BISHOP - 1] & pcols[opponent]; bishops;
       bishops &= bishops - 1) {
    int p = std::countr_zero(bishops);
  ...
  for (U64 rooks = pcs[ROOK - 1] & pcols[player]; rooks; rooks &= rooks - 1) {
    ...
    total += (std::popcount(targets) * rook_mobility_factor * endgame_interp) /
             eg_mg_diff;
  }
  ...
  for (U64 queens = pcs[QUEEN - 1] & pcols[player]; queens;
       queens &= queens - 1) {
    int p = std::countr_zero(queens);
    total += psqvTableLookup(player, p, place_value_queen);
    U64 targets = (mt->rook_magics[p].compute(bb_blockers) |
                   mt->bishop_magics[p].compute(bb_blockers)) &
                  ~opp_pawn_cs;
    total += (std::popcount(targets) * queen_mobility_factor * endgame_interp) /
             eg_mg_diff;
  }
  for (U64 queens = pcs[QUEEN - 1] & pcols[opponent]; queens;
       queens &= queens - 1) {
    int p = std::countr_zero(queens);
    total -= psqvTableLookup(opponent, p, place_value_queen);
    U64 targets = (mt->rook_magics[p].compute(bb_blockers) |
                   mt->bishop_magics[p].compute(bb_blockers)) &
                  ~our_pawn_cs;
    total -= (std::popcount(targets) * queen_mobility_factor * endgame_interp) /
  ...
  int kvals =
      endgame_interp * psqvTableLookup(player, myking, king_middle_game);
  kvals += (eg_mg_diff - endgame_interp) *
           psqvTableLookup(player, myking, king_end_game);
  ...
  kvals -= (eg_mg_diff - endgame_interp) *
           psqvTableLookup(opponent, opking, king_end_game);
  total += kvals / eg_mg_diff;

  // space
  total +=
      std::popcount(our_pawn_cs & central_squares & side_territory[opponent]) *
      space_value;
  total -=
      std::popcount(opp_pawn_cs & central_squares & side_territory[player]) *
      space_value;
...
static U64 see_lvp(U64 attadef, U64 side_pcs, const U64 *pieceBB,
                   enum PieceType &aPiece) {
  for (int p = 0; p < 6; p++) {
...
int Evaluator::see(Position const &pos, enum PieceType piece,
                   enum PieceType target, int frsq, int tosq, int side) {
  side &= 1;
